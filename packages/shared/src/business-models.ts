...
export function storyPromptText(story?: string, name?: string): string {
  return [
    ...
    '- If my words could match more than one model or team, ask; never guess.',
    '- A new model is created with models_create, and only there. Never change an existing model when I asked for a new one, and never create a second one when I asked to change one.',
    '- Read a model back (models_get) before you change it, and only change what I asked.',
    '',
    'The format:',
