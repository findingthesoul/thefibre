...
// certificate_template row; the builder and every renderer share these shapes
// and helpers.
//
// Moved out of apps/thread on 2026-09-25, when the visitor portal needed to
// show a person their own certificate. It is a pure document model — shapes,
// page geometry, and the substitution that turns {name} into a name — with no
// engine and no personal data of its own, which is exactly the half that
// belongs in shared. The RENDERER is separate and also shared (ui/cert-view);
// what stayed behind in apps/thread is the builder, which is organiser
// machinery and has no business in a participant's app.
