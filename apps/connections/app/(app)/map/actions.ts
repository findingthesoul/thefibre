...
export async function loadNeighbourhood(
  personId: string,
): Promise<{ ok: true; neighbours: Neighbour[]; organisations: OrgRef[] } | { ok: false; error: string }> {
  ...
  try {
    const r = await apiFetch<{ neighbours: Neighbour[]; organisations?: OrgRef[] }>(
      `/api/v1/connections/map/${personId}/neighbourhood`,
    );
    return { ok: true, neighbours: r.neighbours ?? [], organisations: r.organisations ?? [] };
  } catch (e) {
...
export async function loadOrganisation(
  orgId: string,
): Promise<{ ok: true; organisation: { id: string; name: string }; members: Member[] } | { ok: false; error: string }> {
  try {
    const r = await apiFetch<{ organisation: { id: string; name: string }; members: Member[] }>(
      `/api/v1/connections/map/org/${orgId}`,
    );
    return { ok: true, organisation: r.organisation, members: r.members ?? [] };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : 'unknown error' };
  }
}
