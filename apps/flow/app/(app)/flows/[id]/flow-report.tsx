...
export function FlowReport({ runs, steps }: { runs: Run[]; steps: Step[] }) {
  ...
  if (total === 0) {
    return (
      <div className="mt-4 rounded-lg border border-line bg-white p-8 text-center text-sm text-ink-subtle">
  ...
  return (
    <div className="mt-4 space-y-6">
      ...
      <div>
        ...
        <div className="rounded-lg border border-line bg-white p-4 space-y-2.5">
          {steps.map((s) => {
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: string }) {
  return (
    <div className="rounded-lg border border-line bg-white px-4 py-3">
      <div className={`text-2xl font-medium tabular-nums ${accent ?? ''}`}>{value}</div>
