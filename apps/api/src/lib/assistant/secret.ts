...
// must be recoverable — by this process and nothing else. AES-256-GCM.
//
...
//     rotation schedules and unrelated blast radii; coupling them would mean
//     rotating SSO_INTERNAL_SECRET silently locked every workspace out of its
//     assistant, and a leak of either was a leak of both.
//   - SSO_INTERNAL_SECRET only as a fallback, so a deployment works before the
//     dedicated secret exists (staging today).
// The first byte of the packed value says which one was used (a key id), so
// a later rotation can re-encrypt row by row instead of asking every
// workspace to connect its key again. Rotating a secret that rows still
// depend on makes those rows unreadable; access.ts treats that as "no key"
// and Settings asks for the key again — the right failure for a credential.
...
function rootFor(kid: number): string {
  ...
  if (!root || root.length < 16) throw new Error(`${name} is not set — cannot protect a workspace model key`);
  return root;
}
