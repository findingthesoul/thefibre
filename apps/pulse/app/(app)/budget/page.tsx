    ...
    <div className="px-6 py-10 max-w-5xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">{t(locale, 'budget_blurb')}</p>
        </div>
        <div className="shrink-0 pt-1">
          <BudgetActions members={members} locale={locale} />
        </div>
      </div>

      <BudgetList lines={items} members={members} locale={locale} />
    </div>
  );
}
