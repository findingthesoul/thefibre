  ...
  return (
    <div className="mt-8 space-y-10">
      ...
      <p className="text-xs text-ink-muted max-w-xl leading-relaxed">
        {t(locale, 'acct_hint_1')} <code className="font-mono">acct_</code>{' '}
        {t(locale, 'acct_hint_2')}
      </p>
    </div>
  );
}
...
function AccountSection({
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (acct && !acct.startsWith('acct_')) {
      setError(t(locale, 'acct_starts_error'));
      return;
    }
    if (showMethods && !stripeOn && !invoiceOn) {
      setError(t(locale, 'keep_one_method_error'));
      return;
    }
      ...
      {disabled ? (
        ...
        <div className="mt-3 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {saved && <span className="text-xs text-ink-subtle">{t(locale, 'saved_notice')}</span>}
          </div>
        </div>
      )}
    </form>
