  ...
  return (
    <div className="flex flex-col gap-4">
      {order.map((id) => {
        ...
        if (!p) return null;
        return (
          <div key={id} draggable onDragStart={() => setDragging(id)} onDragEnd={() => { setDragging(null); setOver(null); }} onDragOver={(e) => { e.preventDefault(); setOver(id); }} onDrop={(e) => { e.preventDefault(); drop(id); setOver(null); setDragging(null); }} className={`relative rounded-lg transition-shadow ${over === id && dragging !== id ? 'ring-2 ring-line-strong' : ''} ${dragging === id ? 'opacity-60' : ''}`}>
