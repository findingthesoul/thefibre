...
| **P4 — writes + ChatGPT** — first slice ✅ v1.50.0 | Sjoerd, 2026-09-25: "Is it now possible to have a chat create a thread based on input?" — "yes please". A `thread:write` scope (asked for by name; a client that names no scopes gets reads only) and two write tools: `thread_create` (blank or from a template, always a draft) and `thread_add_engagements` (a dated list → timeline items, all drafts: rows people attend become agenda items, rows that are sent become fixed-time messages, internal steps become agenda items hidden from the agenda). Plus a prompt, `plan_thread_from_schedule`, that carries that method so a pasted table of dates and steps becomes a thread in one conversation. The consent page names the write scope in words. Still to come in P4: `connections_add_note`, a follow-up setter, ChatGPT. | done |
