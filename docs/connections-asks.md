...
| 111 | "Production does not have the full width... it does show the calendar selection, which I don't see in staging... quite confusing" | **Answered** — production was promoted to v0.82.0 mid-afternoon, which carries the calendar picker but NOT the list shape (that is v0.84.0, staging only). Staging should have both; if it does not, it is a stale page |
