...
| 137 | "the icon of connect (if I download it for my mobile) is the old one" | **Shipped** v1.13.0 — the server was right all along: production serves the new icon byte-for-byte. The service worker PRECACHES the icons and only reinstalls when its OWN bytes change, so phones kept the pre-rename icon out of `connections-shell-v2` — the icons were redrawn 09-22, `sw.js` untouched since 09-13. Bumped to v3, plus a test that fails when a precached file is committed after the worker |
