...
## Where the Fibre suite is right now (2026-07-07, v0.13.108 Â· Thread 3.31.1 Â· Meet 2.4.1 Â· Flow 1.10.0)
...
(docs/invoices-and-roles-proposal.md â all decisions resolved). CLAUDE.md's
"Where we left off" carries the detailed feature inventory; this file keeps
the queue.

### Open queue (in priority order â THE to-do list, keep it current)

_Last groomed 2026-09-06 (v0.52.0). Done items get removed, not ticked._

**0a. Testing roadmap (docs/testing-approach.md + handbook Â§11).** Phase 0
   DONE, Phase 2 started (v0.53.0: pnpm verify gate, 30 unit tests,
   smoke-prod). Phase 1 DONE (CI installed â SSH
   ...
   Ã14 tables, recordPurchase idempotency, sso_handoff race;
   ...
   via a minted /sso/land code â the hop machinery is the E2E session
...
**0. Domain migration aftercare (v0.48.0 hop + v0.52.0 flip are LIVE;
   ...
   *.thefibre.app app subdomains are detached (404) and the transitional
   CORS_ORIGINS secret is removed â Sjoerd confirmed all affected data was
   ...
   - **TransIP cosmetic cleanup** (Sjoerd, no rush): the five stale
     ...
     zone now point at a 404 â delete them whenever convenient.
   - **Decommission old Thread V3** (separate repo ~/Projects/thethread-v3,
     Vercel project thethread-v3) â largely DONE 2026-09-07 (29e9d49; webhook deletion attempted by
     ...
     redeploy emptied the edge cache â the landing "worked" for 22 days on
   ...
   - **Email sending domain**: platform mail still sends from
     ...
     (hello@/certificates@) â reconcile, don't duplicate, if/when app mail
     should come from @thethread.app.
   - Real cross-apex E2E of the hop is now live-testable (thefibre.app â
     app.thethread.app); exercise the full v0.48.0 test matrix.

**Productisation â Sjoerd's two Stripe steps, then the metered phases.**
The plans are decided (docs/pricing-proposal.md), gated (0.19.24), surfaced
and chargeable (0.20.0 + 0.21.0 â docs/productisation-proposal.md is the
umbrella): /admin/plans matrix, Settings â Plan, public /pricing, tailored
...
1. ~~Stripe~~ **DONE 2026-09-03**: live key + all three webhook secrets on
   prod (meet/thread/billing â the July item, finally), sandbox twins on
   staging, live + test Products/Prices synced. â ï¸ Residual: the Stripe
   account showed "capabilities paused â required task overdue" in the
   ...
   just a customer now.
1e. **Event template library (Thread)** â the plan dimension shipped 0.24.3
   (`thread_template_limit`: 1 / 5 / â / â, editable on /admin/plans, shown
...
1f3. **Members page redo (Sjoerd spec, 2026-09-05)**: list rows; click a
   name â settings popup; Add button â invite popup (house pattern, like
   Membership's members page). Same session: **profile convergence** â
...
1g. **Standard components, one implementation (Sjoerd, 2026-09-05:
   "every setting page, profile page, invoice screen â standard platform
...
2. **P4 â meters that bill** (proposal Â§4): ~~seat billing~~ (done 0.22.0 â
...
3. **P5 â website polish**: OG image (favicon shipped), screenshots,
   self-serve signup flip when the trial ends. Now under the naming brief
   (docs/naming-brief.md): Thread-first â NO per-app product pages (Meet /
...
3b. **Thread asks (Sjoerd, 2026-09-05)**: (a) WORKSPACE-scoped threads â
...
3c. **Mobile follow-ups** (bottom menu SHIPPED v0.45.0 to all six apps:
   ...
   desktop-first by design â don't chase touch support there.
4. **Naming brief follow-ups** (docs/naming-brief.md, decided 2026-09-01;
   display renames + Thread-first landing shipped v0.23.0):
   - Meet standalone vs event-type-inside-Thread â PRODUCT decision, Sjoerd.
   - Domain strategy (thread-branded public domain?) â decide with the
     staging build (docs/environments.md Phase 0), one DNS afternoon.
   - /terms + /privacy-policy still carry old names â bundle with the lawyer
     review (queue item 1c), don't edit unreviewed legal text piecemeal.
5. Decisions D1âD6 in docs/productisation-proposal.md Â§5 were built as
   recommended ("continue building, don't wait" â Sjoerd, 2026-09-01, while
...
6. **First-visit onboarding for Meet + Thread** (Sjoerd's refinement note,
   ...
   a first-visit tour offer. Proposal with decisions D1âD3 in
   docs/onboarding-proposal.md â Sjoerd decides, then ~one session to build.
6b. **Multilingual platform** â docs/i18n-proposal.md; D1âD5 decided
   2026-09-05 (D4 overridden: FRENCH NOW). **P1 SHIPPED in v0.41.0**:
   @thefibre/shared ./i18n (six locales incl. fr), Thread catalog+emails
   Ã6, thread.language split into page vs facilitation language,
   Membership public surfaces + lifecycle emails Ã6, certificate emails
   Ã6, membership_settings.locale + membership_member.locale. Same round:
   ONE shared embed integration (@thefibre/shared/embed-loader â both
   ...
   **P2 SHIPPED same day** (split across the two parallel sessions):
   v0.43.0 â identity_profile.locale (NOT user_profile: that's a dead
   read-only fallback since the profile SPoT moved) + Settings â Profile
   language picker + thefibre.locale cookie in savePref + platform
   emails Ã6 (platform-i18n.ts, exports platformEmailLocale â THE
   resolver for platform-side emails); v0.44.0/0.44.1 â all 8
   auth-templates Ã6, auth hook resolves via platformEmailLocale.
   **P3 SHIPPED 2026-09-06** (Sjoerd overrode demand-driven: "P3 for all
   6 languages") in v0.50.0 + v0.51.0: ALL six apps' signed-in
   interfaces Ã6 via per-app lib/i18n-ui.ts catalogs (~3,000 keys
   ...
   documents, raw API errors (P4 never â unchanged). Standing rule: NEW
   user-facing strings always go through a catalog Ã6 â never hardcode
   EN. MT burn-down: `grep -rn '// MT'` is the native-review list
   (NL: Sjoerd; known ES divergence Ajustes/ConfiguraciÃ³n; pt is BR â
   INTL_LOCALES.pt flipped to pt-BR to match).
7. **Membership app (soul.com community)** â **v1 SHIPPED whole in
   v0.31.0** (2026-09-05, docs/membership-proposal.md; D1âD6 accepted):
   7th app, slug `membership` (display name may become **Hyve** â one
   ...
   (activation now really grants app_membership â RLS had no write
   ...
   **Remaining â Sjoerd, not code:**
   - Stripe **Connect** webhook endpoint
     `https://thefibre-api.fly.dev/api/v1/membership/stripe-webhook`
     (checkout.session.completed, invoice.paid, invoice.payment_failed,
     customer.subscription.updated/deleted â MUST pick "listen on
     Connected accounts" AT CREATION, it can't be flipped later) â
     `fly secrets set STRIPE_MEMBERSHIP_WEBHOOK_SECRET`. Was mid-redo
     2026-09-05. Staging twin (test mode â thefibre-api-staging)
   ...
   Â§3.9 as the generalised LOGIC BUILDER (Settings â Pricing rules;
   ...
   docs/spike-circle-sso.md + /api/v1/oauth endpoints â TEST ON STAGING
   before touching Circle's SSO screen. Workspace-threads design:
   docs/brief-workspace-threads.md (D1âD3 await Sjoerd).
   STILL QUEUED: Ã -la-carte product buying; i18n P1 (D1âD5 unread);
   extraction phases 2â4; SearchSelect adoption sweep.
   **Roadmap (proposal Â§3.6):** Memberful-style integrations catalogue â
   each tool = a new access_grant kind + worker (deploy, not migration);
   then org seats (Â§3.5), OAuth provider phase 2, plan-gating +
...
`thread.thefibre.app/developers`, and `scripts/verify-public-api.mjs` â run it
...
privacy policy. **Both legal documents are unreviewed** â a lawyer reading
`/terms` and `/privacy-policy` is item 1c below._

_Flow-as-planner-engine (docs/brief-flow-as-planner-engine.md) is complete
except gap 5, the communities/organisations variation â which the brief itself
says may not belong in Flow at all. That needs a design call before code._

_External apps (docs/brief-external-apps.md) shipped whole in v0.14.0 â open
catalogue + lifecycle, `app_key` with enforced scopes, org links, bulk links,
manifest-validated activity types. Follow-ups from its Â§4 that are still open
are items 10aâ10c below._
...
0. **Wallet issuer credentials (Sjoerd, not code)** â check-in ships with
   ...
   Apple Developer account â `fly secrets set APPLE_WALLET_CERT_PEM
   ...
   Google Wallet issuer account + service account â
...
1. ~~Stripe secrets~~ â DONE 2026-09-03 (see the productisation block at the
...
1b. **Seed the planner's nine steps as a flow** â the platform work is DONE
   ...
   step copy â what's in the planner's `festival-plan.ts` is placeholder (the
   ...
   `write:enrolments`). So the whole arc â plan on Flow, publish on The Thread,
   read who came â is reachable from outside. What remains is content.

   **v0.18.7 removed the last code blocker.** The step copy now exists
   (`~/Projects/festivaloftrust.com/supabase/seed/fot_festival_graph.json` â
   nine steps, eight transitions, 39 default tasks, four `meta` fields each),
   and the flow builder can import it: *Design file* â paste/choose â Check â
...
1c. **Legal review of /terms and /privacy-policy (Sjoerd, not code)** â both
   ...
   both; the source files carry a â ï¸ comment saying so. When the text changes,
   ...
   published on /support â confirm both actually deliver to a human.

2. ~~**Members UI role vocabulary**~~ â **done in v0.18.8.** The web Members
   page was already correct; the stale surface was **Meet â Internal team**,
   whose dropdown posted `'member'` â a value the DB has rejected since
   `20260704090000_role_tiers` â so changing a role there 500'd. Fixed, along
...
3. **Org-share transfers** â thread_payout ledger rows exist ('pending');
   actual Stripe transfers of the workspace share are deferred.
4. **Role-gating beyond Invoices** â enrolments/contacts visibility per the
   tiers (proposal Â§3.8, deliberately out of v1).
5. ~~Certificate email i18n~~ **done in v0.41.0** (Ã6, thread's page
   language). Still open here: `customer_tax_ids` so the Stripe legal
   invoice carries the buyer's VAT number.
6. **Uploads: per-app membership gate** â any workspace member can upload
...
7. **Split apps/api/src/routes/thread.ts (~4.7k lines)** â mechanical
   module split; the full section/dependency map lives in
   docs/thread-split-map.md (2026-07-07). Pure moves only, typecheck
   between steps.
8. **Deduplicate the cross-app frontend** into packages/shared â same play
   ...
   Ã3, displayName join Ã7, `one()` embed-normalize Ã~80, activity-insert
   Ã13 (`lib/activity.ts` â it IS the data wall), admin-role check Ã2,
   slugField Ã2.
8b. **ESLint flat config** â the four `next lint` scripts were zombies (no
   config existed, eslint 9 vs legacy scaffold) and were removed in
   0.13.109; add a real flat config + CI when wanted.
9a. **Curator-data write API** â an external app that wants to annotate a
   ...
   docs/brief-external-apps.md Â§4.
9b. **App-key liveness beyond `last_used_at`** â keys don't expire and nothing
   nags an admin to rotate one. `last_used_at` is shown; that's it.
9c. **Retrofit manifests onto first-party apps** â Meet/Thread/Flow/Pulse
   ...
   apps.
9. **Meet event types** â Group / One-off / Meeting poll stubs in
   new-menu.tsx.
10. **Fibre Pulse â business planner** â LIVE at pulse.thefibre.app,
    v0.13.0 after the 2026-07-07â09 marathon (CHANGELOG 0.13.112â136).
    Everything Sjoerd specced across two days is shipped: sheet-grid
    cashflow (BANK chain, Total column, virtual reserve growth,
    drag/â¥-drag, folds remembered, focus mode, toasts), invoice-style
    opportunity popup (offering rows Ã qty Ã price Ã repeat, VAT
    ...
    anchor), payment-terms curator field (Â§2.5), comparison overlay,
...
11. **Per-event enrolment** (Sjoerd 2026-07-10) â let people enrol in
    ...
    selection vs price-per-event vs access-tickets) â that decision
...
'closed' collapses to draft in the editor (latent â nothing writes
...
### Stripe Connect webhook â DONE 2026-09-05 (was THE revenue blocker)
...
- There are TWO sandboxes: "staging" (the real twin â the staging API's
...
- CHECK: the SL B.V. sandbox may still hold its four junk endpoints
  (meet/billing/thread â PROD api + a misplaced membership one) â if
...
- The "Multiple capabilities paused â required task overdue" banner: if
...
- Staging test-card rehearsal DONE 2026-09-06: EUR 2,000 Soul Fellowship
  ...
  'default' workspace) â webhook 200s, active member, paid ledger row.
...
### Google Workspace integration â SHIPPED v0.47.0 (2026-09-06)

'google_user' grant kind (suspend member's Google account on lapse,
unsuspend on rejoin â never create/delete) + Settings â Integrations
...
parked on the default workspace â RE-ENTER it on the soul.com workspace
...
join page (Sjoerd 2026-09-06: tick-able product options â priced ones
...
## Now â closing the post-deploy loop

- [x] **Tighten CORS** in `apps/api/src/server.ts` â done in v0.13.17. Allowlist covers the 5 thefibre.app subdomains, local dev (3000/3001/3002), opt-in `CORS_ORIGINS` env, and our own `*.vercel.app` previews. Unknown origins get no `Access-Control-Allow-Origin` header (browser blocks). Server-to-server (Stripe webhook, Supabase Send Email Hook) unaffected.
- [ ] **Custom API domain** â `fly certs add api.thefibre.app --config fly.toml`, add the CNAME at the registrar, then update Vercel's `NEXT_PUBLIC_API_BASE_URL` and redeploy. (Web is already at `thefibre.app`.)
- [ ] **Supabase Auth redirect URLs** â confirm `https://thefibre.app/**` and `https://*.thefibre.app/**` are listed (sign-in already works, so likely fine â verify).

---

## Next â feature gaps now that the platform feels real

### Quick wins (under an hour each)
- [ ] **App switcher in the top nav** â dropdown showing The Fibre / Fibre Meet / The Thread (only the apps activated for this workspace + the user has membership for). Surfaces in apps/web's Topbar and apps/meet's header. Each entry links to the relevant subdomain.
- [ ] Activity filter by `organisation_id` â join through `org_membership`. Unblocks org per-app tab timelines (currently EmptyState).
- [ ] Tags â create, assign, filter persons and orgs by them.
- [ ] Person â person relationship form (the `relationship` table already exists, no UI).
- [ ] Microsoft + LinkedIn OAuth providers â Supabase Auth config only.

### Medium (a session or two each)
- [ ] App membership management UI â assign roles + permissions per app per user.
- [ ] Workspace creation + switching (currently one seeded workspace).
- [ ] Invite by email (magic link flow per brief Â§5.5b).
- [ ] Article 15 export â JSON of everything held about you.
- [ ] Article 16 rectification â link from privacy to self-edit fields.
- [ ] Article 20 portability â same payload as export with schema.
...
- [ ] The Thread frontend at `apps/thread/` â events + journeys + sessions. Best-specified in the brief (Â§8 EBBF Athens example). Once one delivery app exists writing activity events back, the full architecture loop closes.
- [ ] Fibre Meet frontend at `apps/meet/` â meetings + agendas + outcomes.
- [ ] Fibre Sales â sovereign app, gated, deal pipeline + handover webhook on `deal_won`.
- [ ] Fibre Learn â future, blocked on a content authoring system.

---

## Phase 2 â programme layer (mostly shipped in v0.4.1)
...
- [ ] Programme detail with per-format content (sessions for events, milestones for journeys) â that's delivery-app territory

---

## Phase 3 â GDPR UX (mostly shipped, see "Medium" above for what's left)
...
## Phase 4 â Fibre Sales (gated app, when ready)

(See "Bigger" above. Schema in Â§5 Domain 8 of v0.3 brief still applies â it's the only delivery app whose schema is fully specified.)

---

## Operational & infra

- [ ] **Custom email domain** â Resend with `@thefibre.app`, SPF / DKIM / DMARC
- [ ] **Lint rule banning Supabase imports under `apps/web/app/api/`** â enforce brief rule Â§13
- [ ] **CI** â typecheck + build on every PR
- [ ] **Backups** â Supabase has them; confirm retention, document restore
- [ ] **Migrations workflow** â staging environment for trying migrations before prod
- [ ] **CORS hardening** on the API once it's public â restrict to production web origins
...
- `apps/api/src/routes/sso.ts`: gated by `SSO_INTERNAL_SECRET` â rotate before prod.
...
- **JWT `sub` â  `public.user.id`.** Use the `app_user_id` claim (the hook injects it).
...
- **`activity` has no `organisation_id`** â org per-app tabs render their curator section but EmptyState the timeline. Future fix via join through `org_membership`.
...
- One feature, one version. SemVer:
  - **patch** (`0.x.y+1`) â additions and fixes that don't change UX shape
  - **minor** (`0.x+1.0`) â UX milestone or new top-level page or schema principle
  - **major** â reserved for breaking API changes once we have external consumers
- Every shipped version updates: `package.json` Ã 4, `apps/web/app/(app)/layout.tsx` (sidebar footer), `CHANGELOG.md`.
- Build plan ticks come *off* when shipped â completed items move out of view here (CHANGELOG keeps them).
...
2. Parent (me) builds the foundation first â layout, stubs, shared API. Agents only fill leaves.
3. After every parallel batch: full `pnpm -r typecheck`, then commit.
4. Sequential is faster for â¤2 tasks. Parallel pays off at 3+.

---

## Parked / decisions deferred

- **`person_app_profile` / `org_app_profile` JSONB extension tables** (brief v0.4 Â§5 Domain 5) â the canonical home for app-owned curator data once schema stabilises. Right now the existing app-tagged tables play that role.
- **Auto-edit / 10-step undo / change history** â good idea, premature. GDPR erasure must zero PII; storing old field values is a hidden second copy. Revisit once we know which fields people actually edit most.
- **Self-hosted Supabase on Hetzner** â migration trigger documented in brief Â§4 (client requiring no-US parent, scale â¥10k users, or sovereign regulator).
- **Fly.io vs Railway** â picked Fly.io. Config in `apps/api/fly.toml`.
- **Region** â project is West EU (Ireland). Both EU, GDPR-compliant. API will deploy to Fly Frankfurt to align with brief intent.
- **GraphQL via Hasura** â only if a contract requires it.
